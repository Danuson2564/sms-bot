...
async def sms(ctx, phone=None, amount=None):
    if (str(ctx.message.channel.id) == '944478465015578634'):
        if (phone == None or amount == None):
            await ctx.send("> กรุณาใส่ข้อมูลให้ครบถ้วน")
            
        else:
            try:
                amount = int(amount)
                if (amount > LIMIT):
                    await ctx.send(f"> กรุณาใส่จำนวนไม่เกิน {str(LIMIT)}")

                else:
                    await ctx.send(f"> เริ่มยิงไปที่เบอร์ **{phone}** แล้ว | เวลา **{amount}**นาที")
                    startall(phone, amount)

            except:
                await ctx.send("> คุณใส่จำนวนไม่ถูกต้อง")
