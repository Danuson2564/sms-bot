...
TOKEN = '' #token ของบอท
LIMIT = 5000000000000000000000000000000000000000000000000000 #จำนวนสูงสุดที่ใส่ได้ต่อการยิง 1 ครั้ง
blacklist = [
    "0000000000",
    "1111111111"
]
...
@bot.event
async def on_ready():
   await bot.change_presence(status=discord.Status.idle, activity=discord.Activity(type=discord.ActivityType.playing, name='LILWIN SMS'))
...
async def help(ctx):
    ...
    embed.set_footer(text=" |SMS EDIT BY LILWIN")
    embed.set_author(name='SMS EDIT BY LILWIN')
...
async def sms(ctx, phone=None, amount=None):
    if (str(ctx.message.channel.id) == '969098345526198312'):
